...
Static Void dumpnodes()
{
  ...
  fprintf(tracefile,
	  "--------------------------------------------------------  ");
  fprintf(tracefile, "%2d/%2d/%2d", datevar.month, datevar.day, datevar.year);
...
/* Note that this is not fully correct since if you are running
   ...
   systems.                                          */
    sprintf(cmdline, "start EPM.EXE %s/log.doc\n",
	    GetChipmunkPath("LOGLIB", LOGLIB));
#else
//   if (*m_display_name == '\0')
    sprintf(cmdline, "edit %s/log.doc & \n", 
	    GetChipmunkPath("LOGLIB", LOGLIB));
//   else
 //   sprintf(cmdline, "xterm -display %s -e %s %s/log.doc & \n",
 //           m_display_name, pager, GetChipmunkPath("LOGLIB", LOGLIB));
#endif /* OS2 */
 ...
 * Note: Using % for possibly-negative arguments [317] */
	...
	WITH1 = readdesc(&GETFBUF(libf1[V.f - 1]->f, filerec).b[16*((i00 - 1) &
                15)]);
...
/* p2c: log.text, line 12164: Note: Can't interpret size in NA_NEW [174] */
	if (WITH->numpnums != 0) {
	  ...
	  for (j = 0; j < FORLIM1; j++) {
	    readint(&V);
            WITH->pnum[j] = readpnum(&GETFBUF(libf1[V.f - 1]->f, filerec).b
                            [4*V.p]);
...
Char *name;
{
  ...
  if (zoom < 2) {
    *buf = '\0';
    ...
    FORLIM = strlen(name);
    for (j = 0; j < FORLIM; j++) {
      j2 = name[j];//strposc(nummap, name[j], 1L);
//      if (j2 > 0)
  ...
  *V.reason = '\0';
  if (*filename != '\0') {
	...
	loadfail("No signal-count line", &V);
      ip = (long *) Malloc(count * sizeof(long));
      for (i = 0; i < count; i++) {
	fscanf(f, "%hd%c", &j, &ch);
	if (ch == '\n')
	  ch = ' ';
	fgets(tempname, 256, f);
	...
	if (ch == '\n')
	  ch = ' ';
	fgets(labelname, log_lablen + 1, f);
...
Char *reason_;
{
  ...
  do {
    ...
    case 'n':   /*nodes*/
      ...
      for (j = 1; j <= i; j++) {
	fscanf(*f, "%ld%ld%*[^\n]", &m, &n);
	getc(*f);   /*simtype, num attrs*/
	for (m = 1; m <= n; m++) {   /*skip attributes*/
	  fscanf(*f, "%*[^\n]");
	  getc(*f);
	}
	while (!P_eoln(*f) && !P_eof(*f)) {   /*skip simtype info*/
	  fscanf(*f, "%*[^\n]");
	  getc(*f);
	}
      }
