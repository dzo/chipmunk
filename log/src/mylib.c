...
#define GrLine(x,y,x1,y1,z)   MoveToEx(hdc,x,y,NULL);LineTo(hdc,x,y)
//#define stderr trace
  ...
  Pfprintf(stderr, "m_readpen(pen) flags=%x, x=%d, y=%d\n", theflags,thex,they);
   if(theflags) { 
    if (theflags & 3) {
      ...
      pen->depressed = (thebuttons & Button1Mask) || pen->dn;
      pen->up = 0;
      ...
      pen->depressed = (thebuttons & Button1Mask) && (! pen->up);
      pen->near_ = ! (thebuttons & Button3Mask) ||
